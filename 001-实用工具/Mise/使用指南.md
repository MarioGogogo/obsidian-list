...
## <mark style="background: #FF5582A6;">自定义任务</mark>
...
[tasks]
# 定义一个叫 "dev" 的任务
dev = { run = "npm run start", description = "启动前端开发服务器" }

# 定义一个复合任务，先跑 build 再跑 deploy
deploy = { depends = ["build"], run = "./scripts/deploy.sh" }

# 甚至可以用 python 写脚本任务！
"script:hello" = '''
#!/usr/bin/env python3
import sys
print("Hello from python managed by mise!")
```



当你在终端输入 `mise tasks` (或 `mise t`) 时，Mise 会打印出这样的表格：

```
$ mise tasks
Name    Source       Description
build   mise.toml    打包生产环境代码
dev     mise.toml    启动前端开发服务器 (Hot Reload)
test    mise.toml    运行所有单元测试

```

## 更有趣的用法：`mise run` 自动补全

如果你配置了 shell 补全（比如 zsh-completions），当你输入 `mise run` 后按 **Tab** 键，Shell 会直接把这些 `description` 显示在补全菜单里！

bash

`$ mise run <TAB> build  -- 打包生产环境代码 dev    -- 启动前端开发服务器 (Hot Reload) test   -- 运行所有单元测试`

这对于有很多脚本的大型项目来说，简直是救星。不用再去翻 `package.json` 猜 `npm run script-a` 到底是干嘛的了。
