...
在 React Native **新架构 (The New Architecture)** 中，原生模块向 JS 发送事件的方式与旧架构截然不同。本文档详细介绍如何配置原生事件系统。

  ![](assets/新架构原生事件配置指南/file-20260118210236259.png)
...
| **TurboModule** | 新架构原生模块，支持强类型和 Codegen |

| **Callbacks** | 回调函数，单次调用 |

| **Promises** | Promise 异步返回 |

| **Continuous Events** | 持续性事件，需显式订阅/取消订阅 |

| **DeviceEventEmitter** | JS 端事件发射器 |
...
## 第一步：定义事件接口 (TypeScript)

  

在 `src/specs/` 目录下创建事件接口文件。
...
继承 Codegen 生成的抽象类，并使用 `JSIModuleRegistry` 发送事件。
...
add(NativeEmitterPackage()) // 注册事件模块
...
setEventLogs(prev => [newLog, ...prev].slice(0, 50)); // 保留最近 50 条
...
| 1 | `NativeEmitter.ts` | 定义事件接口 |

| 2 | `package.json` | 配置 Codegen |

| 3 | `NativeEmitterModule.kt` | Kotlin 实现 |

| 4 | `NativeEmitterPackage.kt` | 模块注册 |

| 5 | `MainApplication.kt` | 注册 Package |

| 6 | `EventDemoScreen.tsx` | 监听和使用事件 |

  

**关键点**：

- 使用 `DeviceEventManagerModule.RCTDeviceEventEmitter` 发送事件
